...
- Meet the same quality, testing, and style standards as any contribution —
  `pre-commit run --all-files` must pass, and code must follow the style
  rules in `CLAUDE.md` (no comments unless the *why* is non-obvious, no
  docstrings beyond a single line, no backwards-compatibility shims).
...
work to is the same one `CLAUDE.md` holds every session to: don't fabricate,
don't guess silently, ask before assuming. A contribution that fails that
standard is held to it whether a human or an AI tool produced the line in
question.
